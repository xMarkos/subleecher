...
# Regex to match:
# 1) show name S01E02 junk
# 2) show name 01x02 junk
# whitespace, dot and underscore act as word separators
rx_episode_info = re.compile(r'^(?P<name>.*?)[\s\._]+(?:S(?P<season>\d+)E(?P<episode>\d+)|(?P<season>\d+)x(?P<episode>\d+)).*$', re.I)

# Non-comprehensive list of tags to be thrown away to get the name of the movie
rx_tags = re.compile(r'\b(\d{4}|[a-z]+rip|\d{3,}p|divx|xvid|[hx]26\d|avc|bluray|dvd|hdtv|repack|proper|web-?dl)\b', re.I)

rx_boundary = re.compile(r'[\s\._]+')
...
class OpenSubtitles(object):
	...
	def _get_hash(self, file_path: str, allow_hash_file=True):
		if allow_hash_file:
			...
			hash_file = next((x for x in hash_candidates if os.path.exists(x)), None)
			if hash_file:
				# Special .hash file that contains 2 lines: OS Hash, file size
				# It is generated by app that reencodes a video file
				with io.open(hash_file, 'rt', buffering=1) as fin:
					return fin.readline(32).strip()

		f = oshash.File(file_path)
		return f.get_hash()

	def _get_episode_info(self, file_path: str):
		file_path = os.path.basename(file_path)

		if m := rx_episode_info.search(file_path):
			name, season, episode = m.group('name', 'season', 'episode')

			name = rx_boundary.sub(' ', name).strip()
			season = int(season)
			episode = int(episode)
	...
	def search_file(self, file_path: str, *, languages: Iterable[str] | str = None):
		...
		if info := self._get_episode_info(file_path):
			args['season'] = info[1]
			args['episode'] = info[2]
